...
Empirical in-game testing on a real modded GRB install is **now a load-bearing source** (it was a gap on 2026-06-30). It established, among other things, that a **raw/uncompressed cloth `.data` crashes/hangs GRB at load** while an Oodle-Mermaid-compressed one loads. It has **not** yet established whether any cloth *parameter* is runtime-effective: every param edit tested (gravity §4357 **and** the dedicated §4398, stiffness §4360, wind §4397, MaxDistance) showed no in-game effect, but all those nulls are **confounded by the forge shadow** (2026-07-03: the tested kilt cloth is duplicated across `DataPC.forge` and a WorldMap base forge, and only the `DataPC` copy was edited) — so nothing is confirmed live *or* inert, pending a shadow-free both-patch-forge override test. These results are recorded in [`meta/research-log.md`](research-log.md) (2026-07-02 / 07-03 entries) and drive [`docs/11-cloth-and-physics.md`](../docs/11-cloth-and-physics.md). **Reliability:** direct observation on one researcher's machine — high for what was seen, but single-install.
