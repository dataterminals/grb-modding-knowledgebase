...
| Prefix | Meaning |
|---|---|
| `WG_` | **W**eapon **G**ameplay — weapon-group gameplay/assembly **definitions** (route to `DataPC_extra`). *(expansion inferred; aligns with [`glossary.md`](glossary.md))* |
| `WI_` | **W**eapon **I**tem — inventory / item **definitions** (route to `DataPC_patch`). *(expansion inferred)* |
...
A weapon reskin typically ships a **`WG_`** (gameplay) + **`WI_`** (item) **definition** pair — routed to `DataPC_extra` and `DataPC_patch` respectively — with the actual component meshes over in `DataPC_Resources`. Worn-gear *meshes* commonly ship as a **`TP_`/`FTP_` pair** (covering gender/viewmodel variants). Textures often ship as a **base map + `_Mip0` pair** (GRB splits a texture's streaming body from its top mip; the two carry *different* resource IDs — e.g. `Deckard_ATACSWeaponPaintReplacement`'s ArcticTundra base `63552` / Mip0 `65655`).
...
**(a) Reuse of real vanilla IDs (in-place override).** The replacement resource carries the **same embedded 64-bit `ClassID`** as a stock vanilla entry, so on repack ATK writes it with that ID and it overrides the vanilla resource. Modders often *also* echo that ID in the filename prefix for clarity, but the override is keyed on the **embedded ClassID** ATK reads from the bytes — **not** the filename number (a positional/sort label). See [`glossary.md`](glossary.md) "File ID".
...
**(c) Placeholder filename labels (new-asset injection).** Round-number labels mark *freshly imported* assets that don't correspond to any vanilla resource. These are **filename labels, not IDs** — there is **no repack-time ID reassignment**: each new file already carries its own distinct embedded `ClassID` in its bytes, and *that* is its real ID (verified — two `77777_-_…` files have *different* embedded ClassIDs). The label is just a human marker on the working file:
...
- **`1_-_` round-number labels** — occasionally used for a bulk imported mesh library (`APC9`'s ADP9 set). *(The real ID of each is its embedded `ClassID`; the leading number is only a sort label — no reassignment happens at repack.)*
...
>
> **⚠️ Correction (2026-07-02):** an earlier version of this note claimed *"append-style/binary forge edits are not launch-safe because GRB runs a pre-engine forge-structure integrity check."* **That theory was wrong and is retracted.** The real cause of the launch failures we hit was the **`.data` block format, not the forge-packing method:** a cloth `.data` written with **raw/uncompressed** blocks makes GRB crash/hang at load, while the *same* edit written **Oodle-Mermaid-compressed** loads fine. **Both** a hand-written append-repoint forge writer **and** a clean ATK repack fail identically with raw-block entries and both succeed with compressed ones — so the packing method was never the issue, and no "pre-engine forge integrity check" was ever demonstrated. Practical upshot is unchanged (**use ATK repack**), but for the right reason: keep edited `.data` **compressed** (see [`../docs/02-forge-file-format.md`](../docs/02-forge-file-format.md); [`meta/research-log.md`](../meta/research-log.md), 2026-07-02).
>
> **⚠️ Narrowed (2026-09-16):** that rule was established on **cloth** and is **not universal**. The gameplay `DBContainerEntry_0X104634F921.data` in a modded `DataPC_patch_01.forge` is **entirely raw** — all 27 meta and 1,734 file blocks uncompressed, 57.7 MB where the pristine Ubisoft copy is 13.9 MB compressed — in an install that is played with those mods. ATK's own repack is what dropped the compression. So "raw blocks hang the load" holds for cloth and — on the modder's report that this install plays, not on an observed launch — does not hold for that container; where the boundary lies is unestablished. See [`../docs/14-ai-and-npc-behaviour.md`](../docs/14-ai-and-npc-behaviour.md) §8.
...
- **`.cloth` / MotionCloth.** *Not present* in any of these 18 mods — the vests/pants here are handled as static/rigged `TP_`/`FTP_` meshes, not physics cloth. Physics-cloth gear is a separate resource family that lives in `DataPC.forge` — **and, for ~44 of 56 cloths, is *shadowed* by a same-ID copy in a WorldMap base forge (`DataPC_TGT_WorldMap_Bootstrap_Split.forge`), so a cloth override must be repacked into *both* families' patch forges (a single-patch override hangs the load; vanilla ships `TP_Top_Bodark_Trench_Cloth` exactly this way).** See [`../docs/11-cloth-and-physics.md`](../docs/11-cloth-and-physics.md) and the forge shadow in [`../docs/06-game-load-and-reassembly.md`](../docs/06-game-load-and-reassembly.md); reskinning cloth is a distinct, harder job than a normal mesh swap.
