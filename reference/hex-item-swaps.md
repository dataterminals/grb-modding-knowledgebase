...
> **⚠️ Offset caveat (unresolved).** spncryn says "bytes 1-8", i.e. the first eight bytes of the unpacked resource file. But [`resource-type-ids.md`](resource-type-ids.md) documents a resource payload as `[FileHeader][uint64 ClassID][uint32 Extension]…`, where `FileHeader` is normally one `0x00` byte — which would put the ClassID at offset **1**, not 0. Either these DB resource types write no header byte, or "bytes 1-8" is being used loosely for "the first eight bytes." **Check your specific file rather than assuming an offset**; the ID is unmistakable once you know the `… 01 00 00` tail shape.
