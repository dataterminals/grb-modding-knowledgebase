...
| **Cloth** (MotionCloth / `ClothPackage`) | Physics cloth simulation data (capes, coats, straps). **Format reverse-engineered** — see [`docs/11-cloth-and-physics.md`](../docs/11-cloth-and-physics.md) and [`cloth-section-types.md`](cloth-section-types.md). | ⚠️ **ATK's cloth features are gated off for GRB** (`SoftBody.SupportedGames` excludes GRB): **no** Mesh-Viewer / GLB / XML. ATK round-trips GRB cloth only as **opaque bytes** on repack. Edit at the raw-section level with [`motioncloth.py`](../tools/motioncloth.py) → compressed `.data` repack. *(The XML "tune gravity/stiffness/wind" workflow holds for ATK's supported games, not GRB. And note: editing the per-cloth gravity fields showed **no in-game effect**, but that result is **confounded by a forge shadow (2026-07-03) and unresolved** — see [`docs/11`](../docs/11-cloth-and-physics.md).)* | ⚠️ bytes only |
| **SoftBody** / **MotionSoftBody** | Soft-body physics deformables; `SoftBody` carries `ClothGenerationSettings` (generate cloth from a mesh) — **for ATK's supported games only**. | ⚠️ **not for GRB** — the SoftBody path (Mesh-Viewer / GLB / BuildTable-gen / cloth-generation) is gated off for GRB (`SoftBody.SupportedGames`). | ⚠️ no |
