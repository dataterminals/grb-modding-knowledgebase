...
- **Watch for filename collisions when copying mod files into `Extracted\`.** Entries unpack as `<N>_-_<Name>.<ext>`; copying in a mod file whose exact filename already exists silently replaces that entry. The community fix is to renumber incoming mod files to `1_-_` (and optionally append a keyword) before copying. This is a **disk-level** safeguard only — it does not change what your mod overrides in-game, which is keyed on the embedded `ClassID`. See [`08-naming-conventions.md`](08-naming-conventions.md).
