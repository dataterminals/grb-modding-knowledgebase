...
4. For new content, **mint IDs** (Hash Converter) and create the entries; generate/patch the **BuildTable** so the game knows how to assemble your asset. The Mesh Viewer can export a BuildTable from a scene/mesh/skeleton. To edit one by hand, double-click it in ATK to get an XML export and right-click → **Compile** to go back — the format, and the standard "move a mod to another gear slot" copy-paste, are documented in [`../reference/buildtable-xml.md`](../reference/buildtable-xml.md).
...
- **Nested containers repack inside-out.** Gear BuildTables live in the `TEAMMATE_Template` container and weapon DB records in `Dbcontainer` — both are `.data` containers *inside* `DataPC_patch_01.forge`. Repack the **inner container first and let it finish**, then repack the outer forge; otherwise the forge is rebuilt around a stale container. (Community standard practice — see [`../reference/mod-anatomy.md`](../reference/mod-anatomy.md).)
- **Watch for filename collisions when copying mod files into `Extracted\`.** Entries unpack as `<N>_-_<Name>.<ext>`; copying in a mod file whose exact filename already exists silently replaces that entry. The community fix is to renumber incoming mod files to `1_-_` (and optionally append a keyword) before copying. It does not change *which* resource your mod overrides — that is keyed on the embedded `ClassID` — but it does decide the repack: **when two files in the folder carry the same `ClassID`, ATK packs the lowest-numbered one and silently drops the rest** (verified in ATK's source and on a real install, 2026-09-16). A mod copy numbered above the vanilla copy is thrown away. See [`08-naming-conventions.md`](08-naming-conventions.md).
- You repack the forge that holds the target entries — usually the `*_patch_01` set for gear/weapons (see [`05-three-forge-model.md`](05-three-forge-model.md)). (Cloth is a special case: it lives in the base `DataPC.forge` **and**, for ~44 of 56 cloths, is *shadowed* by a copy of the same ID in a WorldMap base forge — `DataPC_TGT_WorldMap_Bootstrap_Split.forge`. Overriding a shadowed cloth requires a patch entry in **both** families' patch forges; a `DataPC_patch_01`-only cloth override hangs the load. See [`11-cloth-and-physics.md`](11-cloth-and-physics.md) and the forge shadow in [`06-game-load-and-reassembly.md`](06-game-load-and-reassembly.md).)
