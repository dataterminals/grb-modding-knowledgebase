...
4. Re-encode the sections: group additional verts **by their sim triangle** to build the CSR `TriangleVerticesCount` (4562, `byte[tri]`) + `TriangleFirstVertexIndex` (4563, `ushort[tri]`); for each additional vert store two of its three triangle-barycentric weights at the fixed vertex slots (quantize `byte = round((w − Offset)/Scale)`, pack hi/lo into one `4565` ushort, third weight derived on decode); set `4561 {N = tri count, M = A padded to ×8}` and pad `4565` with `0xFFFF`; write `4564 {Scale = maxStoredWeight/255, Offset ≈ 0, Magic = Scale·255}`. Leave the sim mesh, constraints, and per-vertex data untouched. *(The one open detail — which two slots and their byte order — is a fixed convention to confirm against the render mesh or by in-game trial.)*
