...
> 2. **The per-cloth `ClothProperties.Gravity` (section 4357) is IGNORED at GRB runtime.** Reversing *and* zeroing it on a loose Tactical Kilt produced **zero** visible change in-game (edit confirmed live, not patch-shadowed). A **dedicated `ClothPropertiesGravity` (section 4398)** holds the same value and is the untested **live-candidate** (the Step-1 experiment in [`../meta/next-session.md`](../meta/next-session.md)). So treat "tune `ClothProperties` and the cloth changes" as **not established for GRB** — the runtime likely reads the *dedicated* sibling sections (4398 gravity, 4397 wind, 4360 stiffness), not the 4357 struct fields.
>
...
| **`Gravity`** | Vec3 | (0,0,-10) | gravity vector (−Z = down). ⚠️ **This copy is inert at GRB runtime** — the live value (if any) is the dedicated `ClothPropertiesGravity` section **4398**. |
...
1. **Tune existing cloth (research lever, runtime-effect under test):** ATK **cannot XML-export GRB cloth**, so tuning is done by editing the raw MotionCloth sections directly ([`../tools/motioncloth.py`](../tools/motioncloth.py)) and repacking a **compressed** `.data`. ⚠️ **Which tunables actually affect GRB at runtime is only partly known:** `ClothProperties.Gravity` (section 4357) is **verified inert** — edit the **dedicated** sibling sections instead (`ClothPropertiesGravity` 4398, `ClothPropertiesWind` 4397, `ClothPropertiesConstraintsStiffness` 4360), whose runtime effect is being tested (Step 1, [`../meta/next-session.md`](../meta/next-session.md)). Not yet a reliable "too stiff/floppy" knob — it's the current frontier.
...
> **Bottom line for Sami:** the cloth didn't take on because `Cloth_WalkerCoat` is bound to the *vanilla* coat's vertices, not to a name — your new mesh has different vertices, so the baked mapping is invalid. The reliably-achievable win today is **reshape the vanilla garment while preserving its sim-cage vertex count and order** (constraints are index-addressed). Parameter *tuning* is possible only via raw-section edits + a compressed repack, and **which parameters actually move GRB cloth at runtime is still under test** (gravity's 4357 field is inert; the dedicated 4398/4397/4360 sections are untested). Adding cloth to genuinely new topology needs a render↔sim rebind step that isn't solved for GRB. *(Note: the XML/GLB cloth workflows this doc references are ATK's capability for its **supported** games — they don't run for GRB.)*
...
>
> **⚠️ Update (2026-07-02):** further testing sharpened two things here. (a) The ghillie strands are **pinned** (`VertexMaxDistance ≈ 0`) — they move from skinned motion, not free sim — so they were a poor wrap subject regardless. (b) The "run the `--gravity` control first" advice is now unreliable: the `ClothProperties` (4357) gravity field this refers to is **verified inert at runtime**, so it can't serve as a positive control. Gravity testing has moved to the **dedicated section 4398** (see the status banner at the top and [`../meta/next-session.md`](../meta/next-session.md)). The wrap-as-render-driver question remains **in-game unvalidated**.
>
