...
> **⚠️ Runtime-effectiveness caveat (GRB) — UNRESOLVED.** The fields below are the *authored* tunables, verified from `ClothProperties.Read`. But **whether editing any of them changes GRB at runtime is unconfirmed.** Every parameter edit tested on the kilt — gravity (both the §4357 `ClothProperties` field *and* the dedicated §4398 `ClothPropertiesGravity`), stiffness (§4360), wind (§4397), and MaxDistance — showed **no in-game effect**, but **all of those nulls are confounded by the forge shadow** (2026-07-03): the kilt cloth is duplicated across `DataPC.forge` and a WorldMap base forge, and only the `DataPC` copy was ever edited (see the status banner at the top). So treat neither the §4357 struct fields **nor** the dedicated sibling sections (`ClothPropertiesGravity` 4398, `ClothPropertiesWind` 4397, `ClothPropertiesConstraintsStiffness` 4360 — see [`../reference/cloth-section-types.md`](../reference/cloth-section-types.md)) as confirmed live *or* inert. The decisive shadow-free test — a complete **both-patch-forge** override — is staged but not yet run; see [`../meta/next-session.md`](../meta/next-session.md).
...
> **How the paint is stored on GRB (verified 2026-07-03).** ATK's `MotionSoftBodyLOD` reader (for its *supported* games) expects the 6 per-sim-vertex paint arrays — `VertexMaxDistance`, `BackStop`, `GravityScale`, `Damping`, `SkinWidth`, `Friction` (in that order, MaxDistance first) — **length-prefixed**. GRB writes them **BARE instead: `6 × byte[V]` contiguous** (no length prefixes), the whole block ending exactly at the `ClothPackage`'s trailing `int32 blobLen`. So to locate/edit `VertexMaxDistance` in a GRB cloth, take the **first `V` bytes** of that `6·V` run. Verified on both Tactical Kilt cloths and both their LODs. (A spatial correlation to the §4363 sim positions was weak, so the paint uses a different vertex order than the position buffer.)
...
> ## ✅ DECODED 2026-09-18 — the render↔sim wrap, completely
>
> **The static blockers below are closed.** The stored wrap is a **per-render-vertex barycentric mesh
> mapping** for four attributes (position, normal, tangent, binormal). It rebuilds the real render mesh from the
> sim cage to a median of **0.45 mm** across **87 distinct cloth LODs / 489,472 vertices** (Walker LOD0:
> 0.38 mm, normal 3.8°, tangent 2.9°, binormal 1.6°). Every structural claim holds on **all 156** MotionBodies.
> Tool: [`../tools/clothmap.py`](../tools/clothmap.py). Provenance: the 2026-09-18 research-log entry.
>
> The block after each LOD's `ClothPackage` (after the TriQuad / sim positions / sim normals / sim indices
> arrays, an empty list and `i32 renderCount`):
>
> ```
> +0    u32 1, u32 0 ×12
...
> +64   f32 min_uv[4] · scale_uv[4] · min_h[4] · scale_h[4]      (position, normal, tangent, binormal)
> +128  u16 table[renderCount]  record index per render vertex, 0xFFFF = skinned only; padded to 4 B
>       record × bound, 20 B:   u8 u[4] | u8 v[4] | u8 h[4] | u16 sim[3] | u16 1
> ```
>
> - `value = min + byte × scale`, weights **`(u, v, 1 − u − v)`** on `(sim[0], sim[1], sim[2])`.
> - **position** `= Σ wᵢ · (xᵢ + h · nᵢ)`, with `nᵢ` the cage's vertex normals from the same arrays.
> - **normal / tangent / binormal** `= unit(Σ w'ᵢ · xᵢ + h' · unit(Σ wᵢ · nᵢ) − position)`. Each direction
>   point sits at unit distance from the position.
> - **The table is the record↔render-vertex map**, and records are in render-vertex order. The ~547 "0xFFFF
>   delimiters" read on 2026-07-01 are the 548 vertices the cloth does not drive.
> - The MotionBody stores the same numbers again (§4374, §4376–4380, §4386, §4403–4410; see
>   [`../reference/cloth-section-types.md`](../reference/cloth-section-types.md)). **A rebind must rewrite
>   both copies.**
> - ⚠️ ATK's `Vertex.Position` is **before** `Mesh.Scale` (2.0 on these garments). Multiply, or the render
>   mesh sits at half size against its own cage.
>
> ### ⭐ Vanilla already contains a rebind
>
> `TP_Top_Bodark_Trench_Cloth` (Kropotkine) carries a sim cage **byte-identical** to Blake's
> `IanBlake_TrenchCoat_Cloth`, still named `Sim_Tsec_IanBlake_Trench_LOD0`. Its mapping, though, is
> **regenerated** for Kropotkine's own mesh. §4386 names `TP_Top_Bodark_Trench_LOD0_VIS`, 0 of 2,243
> records match Blake's, and it rebuilds Kropotkine's mesh to 0.42 mm. **Ubisoft put one garment's cloth
> physics onto another mesh by keeping the cage and regenerating the mapping.** That is the project goal's
> exact operation, and it tells a rebind which bytes to write: the mapping, not the cage.
>
> *What this does NOT settle:* whether a mapping **we** write is accepted at runtime. Lane 2 STEP 1 (does a
> modified cloth load at all?) still gates shipping. The decode makes a no-launch test possible first:
> re-encode a vanilla mapping and compare it with the shipped one.
...
> - **The render mesh is an ordinary skeleton-skinned mesh.** Its per-vertex skinning is to a **skeleton**, not to the 170 sim vertices, so the render mesh does **not** store a per-vertex binding to the sim mesh. *(⚠️ **Corrected 2026-09-01:** this originally said the skinning was **two-bone** with indices spanning **0..23**. It is **four-influence** — `Joint4` at vertex bytes 24–31, four indices then four weights, `{1: 490, 2: 53, 3: 238, 4: 1035}` on LOD0 and indices 1..25 against a 30-bone list. The old reading parsed bytes 32–35, which are a **colour** channel. Verified against ATK's own reader running headless — see the 2026-09-01 research-log entry and [`../tools/atk_bridge.py`](../tools/atk_bridge.py). **The conclusion in this bullet is unaffected**: it rests on 1816 ≠ 170 + 62, which still holds.)*
...
4. ~~**The ragdoll bone-collider list** found in the wearable cloth's editor data.~~ — **answered 2026-08-14; what it names verified 2026-09-16.** Extracted in full from `TP_WalkerCoat_Cloth`: a **550-char, semicolon-separated list of 13 collider names**, each `‹ragdoll›_‹Bone›_‹CRC32(Bone)›` (`TP_WalkerCoat_Ragdoll_Head_130111906` ×2, then `…_LeftArm_3951233754`, `…_LeftForeArm_2310617728`, `…_LeftHand_3061183340`, `…_LeftShoulder_759586984` ×4, `…_Neck_2149808493`, `…_RightArm_1806956327`, `…_RightForeArm_1918345642`, `…_RightHand_1979272496`). They cover the **upper body only** — no spine, pelvis or legs. It is cloth **collision geometry**, not a character binding and not a death-ragdoll definition. **What the names point at (verified 2026-09-16, census of nested resources):** the `TP_WalkerCoat_Cloth.data` container holds a `LiteRagdoll` resource, **`TP_WalkerCoat_Ragdoll`** (3,573 B), beside the `Cloth` and its `SoftBodySettings`, and the cloth references it by ClassID. That ragdoll is a **whole-body set of 23 bone capsules**. The list selects exactly its 13 upper-body capsules, multiplicities included (`Head` ×2, `LeftShoulder` ×4), and leaves out the 10 on legs, hips, spine and holster. GRB ships 88 distinct `LiteRagdoll`s; 19 are garment collider sets like this one, each referenced by `Cloth` resources. *(Corrected 2026-09-16: the 2026-08-14 write-up said "536-char", dropped the `TP_WalkerCoat_` prefix from the first name, and said GRB ships no `LiteRagdoll` — the sweep behind that could not see nested resources.)* See [`../reference/skeleton-reflex3-physics.md`](../reference/skeleton-reflex3-physics.md) and the 2026-09-16 (night) research-log entry.
...
7. **Empirical validation (UNRESOLVED — confounded by the forge shadow):** every cloth-parameter edit tested in-game — gravity (§4357 *and* the dedicated §4398), wind (§4397), stiffness (§4360), and the per-vertex **`MaxDistance`** paint — showed **no effect on the kilt**, but all of those nulls are **confounded by the forge shadow** (2026-07-03): the cloth is duplicated across `DataPC.forge` and a WorldMap base forge, and only the `DataPC` copy was edited, so none of these is established as live *or* inert. (Also settled: there is **no ATK XML path** for GRB cloth, so "XML-tuned `ClothProperties`" was never the test — edits go through raw-section tools + a compressed `.data` repack.) The decisive shadow-free test is a **complete both-patch-forge override** (staged, not yet run) — see [`../meta/next-session.md`](../meta/next-session.md).
