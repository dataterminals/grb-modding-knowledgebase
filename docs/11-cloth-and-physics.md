...
>
> **Net (updated same day — the mechanism is now FOUND): the render↔sim wrap is STORED in the cloth.** Each cloth LOD carries, *after* its sim `ClothPackage`, a large per-LOD block (~35 KB for Walker LOD0) holding the render mesh's data — including a **per-render-vertex binding to ~3 nearby sim vertices** (fixed ~20-byte records; the render vertex count 1816 prefixes it). Evidence: the region holds ~5833 sim-valued indices ≈ 1816 render verts × 3; parsing at a 20-byte stride, the 3 sim indices per record track render order and form **tight local triangles** (median spread ≈ 1.5 sim-edge-lengths, vs. mesh-scale for random triples). So the render mesh follows the sim mesh via a **stored wrap** (render vert → local sim triangle + weights), *not* runtime proximity. This is the real render↔sim binding — the `4561–4565` "additional vertices" are a separate small collision set. **Still to decode:** the exact record layout (weight encoding; ~25% don't parse at the fixed stride → possibly variable-length) and the render-vertex-order remap between the cloth's ordering and the render mesh's buffer. See [`meta/research-log.md`](../meta/research-log.md) (2026-07-01 "hunt the wrap" entry). The subsections below are kept for the verified 4561–4565 section-format details; their *interpretation* ("additional = render verts") is superseded.
