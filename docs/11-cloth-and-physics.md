...
4. ~~**The ragdoll bone-collider list** found in the wearable cloth's editor data.~~ — **answered 2026-08-14.** Extracted in full from `TP_WalkerCoat_Cloth`: a **536-char, semicolon-separated list of 13 collider names**, every one prefixed **`TP_WalkerCoat_`** (`Ragdoll_Head_130111906;TP_WalkerCoat_Ragdoll_Head_…;…_LeftArm_…;_LeftForeArm_…;_LeftHand_…;_LeftShoulder_…×4;_Neck_…;_RightArm_…;_RightForeArm_…;_RightHand_…`). They are **garment-owned capsule colliders named after the bones they follow**, covering the **upper body only** — no spine, pelvis or legs, which is what a coat needs and not what a ragdoll rig looks like. So it is cloth **collision geometry**, not a character binding and not a death-ragdoll definition. *(~~GRB ships no `LiteRagdoll` resource at all~~ — **corrected 2026-09-16:** the `TP_WalkerCoat_Cloth.data` container itself holds a `LiteRagdoll` resource named **`TP_WalkerCoat_Ragdoll`** (3,573 B), beside the `Cloth` and its `SoftBodySettings`. The collider names above almost certainly belong to it — *inferred from the shared name; its body is not decoded*. It was invisible until the container walker stopped reading one byte early; see [`../reference/skeleton-reflex3-physics.md`](../reference/skeleton-reflex3-physics.md) and the 2026-09-16 research-log entry.)*
