...
1. **How does a GRB MotionCloth drive its render mesh? — MECHANISM FOUND (stored wrap); exact format still to decode.** The render mesh is a separate, larger, skeleton-skinned mesh (Walker LOD0: 1816 verts vs 170 sim). The cloth stores the render↔sim binding as a **per-render-vertex wrap** in its per-LOD block after the sim `ClothPackage`: each render vert → ~3 nearby sim vertices (+ weights), ~20-byte records, verified by the local-triangle geometry test (see the "Net" note above). Remaining: decode the exact record layout (weights; the ~25% that don't parse at a fixed stride) and the cloth↔mesh render-vertex-order remap — then a reskin encoder can regenerate the wrap (nearest sim triangle + barycentric) for a new render mesh, which is ATK's `computeTriBarycentricCoords` math. Verified along the way: the GRB render `Mesh` format is parseable (`ClusteredMeshData`, `Pos3s` stride-36, int16×QuantizationFactor, two-bone skeleton skinning).
