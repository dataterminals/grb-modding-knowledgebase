...
>
> **⚠️ IN-GAME UPDATE (2026-07-01, later): editing these records had NO visible effect — treat "wrap = render driver" as UNCONFIRMED.** A live test (twist + full collapse of ghillie wraps, verified applied to the running forge) produced zero visible change in menus or gameplay-with-movement. So the records are geometrically a render→sim binding but probably **not** the runtime render path (more likely editor/build/derived data). Caveat: only ghillies were tested, and ghillie strands may be skeleton-skinned; a *coat* test would disambiguate. See [`meta/research-log.md`](../meta/research-log.md) ("IN-GAME TEST" entry). The claim below is retained for the static findings but is downgraded to unverified.
>
