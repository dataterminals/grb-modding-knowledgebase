...
- Inside (decompressed), each resource record is `uint32 TypeId · int32 Length · int32 NameLength · Name · FileHeader · Payload`. The **FileHeader is counted by neither length** — one `0x00` byte normally, `8 + 12N` bytes when it starts `0x01` — and the `Length`-byte payload begins `[uint64 ClassID][uint32 Extension]…`. *(Corrected 2026-09-16: this line used to put the FileHeader inside the payload. A walker built that way reads every resource after the first one byte early — which is why multi-resource containers such as `TEAMMATE_Template` long looked like one resource. A `.data` holding several resources is the normal case, about 2.5 per container.)* **`Extension` == `TypeId`**, and it is the resource's type — a `CRC32` of the type-name string (e.g. `CRC32("BuildTable") = 585940579`, `CRC32("Mesh") = 1096652136`). ATK reverse-resolves it to the extension you see on unpack. Full id↔type table: [`reference/resource-type-ids.md`](../reference/resource-type-ids.md).
