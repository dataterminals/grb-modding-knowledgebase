...
`Version != Game.Mirage`, so for GRB it keeps the blob as an opaque Base64 lump.
    ...
    record := u8 type | (H(type)-1) header bytes
...
def main(argv):
    ...
    if phys:
        ...
        print(f"    {'#':>4}  {'swing limits (degrees)':<34} {'gravity':>8}  {'damping/stiffness':<26}")
        for n, r in enumerate(phys if raw_mode else phys[:20]):
            ...
            print(f"    {n:>4}  {lim:<34} {r.get('gravity', float('nan')):>8.3f}  {extra:<26}")
